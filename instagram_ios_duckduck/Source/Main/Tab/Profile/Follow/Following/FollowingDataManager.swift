...
class FollowingDataManager{
    func GetFollowing(delegate: FollowingViewController, token: String?) {
        AF.request("\(Constant.BASE_URL)/api/users/following", method: .get,parameters: nil,headers:  ["X-ACCESS-TOKEN": token!])
            .responseDecodable(of: FollowingResponse.self) { response in
                switch response.result {
                case .success(let response):
                    // 성공했을 때
                    if response.isSuccess, let result = response.result {
                        delegate.didSuccessGetFollowing(result: result)
                    }
                    // 실패했을 때
                    else {
                        print("실패 코드에 따라 나누기")
