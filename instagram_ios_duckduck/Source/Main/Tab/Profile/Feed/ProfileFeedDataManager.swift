...
class ProfileFeedDataManager{
    func GetProfileFeed(delegate: ProfileViewController, token: String?) {
        AF.request("\(Constant.BASE_URL)/api/posts/user", method: .get,parameters: nil,headers:  ["X-ACCESS-TOKEN": token!])
            .responseDecodable(of: ProfileFeedResponse.self) { response in
                switch response.result {
                case .success(let response):
                    // 성공했을 때
                    if response.isSuccess, let result = response.result {
                        delegate.didSuccessLoadFeedProfile(result: result)
                    }
                    // 실패했을 때
                    else {
                        print("실패 코드에 따라 나누기")
